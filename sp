...
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/minhhau207-hiro/W-Azure/refs/heads/main/ui%20lib.txt"))()
...
local PriorityTasks, DefaultPriority, TargetGroupId, BannedRanks, NewItemsBuffer
do
    PriorityTasks = {"Boss", "Pity Boss", "Summon [Other]", "Summon", "Level Farm", "All Mob Farm", "Mob", "Merchant", "Alt Help"}
    DefaultPriority = {"Boss", "Pity Boss", "Summon [Other]", "Summon", "Level Farm", "All Mob Farm", "Mob", "Merchant", "Alt Help"}

    TargetGroupId = 1002185259
    BannedRanks = {255, 254, 175, 150}
...
do
    Remotes = {
        ...
        EnchantResult = GetRemote(RS, "Remotes.EnchantResult"),
    }
end
...
if Modules.SummonConfig and Modules.SummonConfig.Bosses then
    ...
    table.sort(Tables.SummonList)
        -- Dropdown update unavailable with direct Options state
    end
...
local function SafeLoop(name, func)
    return function()
        ...
        if not success then
            Library.CreateNoti({
    Title = "Silver Hub (Debug)",           -- tiêu đề (nếu nil thì mặc định "Sea Hub")
    Desc = "Error in ["..name.."]: "..tostring(err),  -- nội dung thông báo
    ShowTime = 10          -- thời gian hiển thị (giây), mặc định 10
})
...
Remotes.UpInventory.OnClientEvent:Connect(function(category, data)
    ...
    if category == "Items" then 
        ...
        -- Owned trade item dropdown update unavailable with direct Options state
    elseif category == "Runes" then
        ...
        for _, dd in ipairs(runeDropdowns) do
            if Options[dd] then 
                local currentSelection = Options[dd] or ""
                -- Dropdown values update unavailable with direct Options state
                if currentSelection and currentSelection ~= "" then
                    -- Dropdown select update unavailable with direct Options state
                end
            end
        end
    elseif category == "Accessories" then
        ...
        -- Accessory dropdown update unavailable with direct Options state
    ...
    elseif category == "Sword" or category == "Melee" then
        ...
        -- Weapon dropdown update unavailable with direct Options state
        ...
        -- Passive weapon dropdown update unavailable with direct Options state
    end
end)
...
Remotes.StockUpdate.OnClientEvent:Connect(function(itemName, stockLeft)
    ...
    if stockLeft == 0 then
        Library.CreateNoti({
    Title = nil,           -- tiêu đề (nếu nil thì mặc định "Sea Hub")
    Desc ="[MERCHANT] Bought: " .. tostring(itemName),  -- nội dung thông báo
    ShowTime = 3          -- thời gian hiển thị (giây), mặc định 10
})
    end
end)
...
local function PostToWebhook()
    local url = Options.WebhookURL
    if url == "" or not url:find("discord.com/api/webhooks/") then return end

    local selected = Options.SelectedData
local allowedRarity = Options.SelectedItemRarity or {}
    ...
    if Toggles.PingUser then payload["content"] = (Options.UID ~= "" and "<@"..Options.UID..">" or "@everyone") end
...
local function UpdateSpecPassiveLabel()
    ...
    local text = ""
    local selectedWeapons = Options.SelectedPassive or {}
    local hasAny = false
...
local function Func_NoGameplayPaused()
    while Toggles.NoGameplayPaused do
    ...
    pcall(function()
        ...
        task.spawn(function()
            for i, v in pairs(workspace:GetDescendants()) do
                if Toggles.FPSBoost and not Toggles.FPSBoost then break end
                pcall(function()
...
local function ApplyIslandWipe()
    if not Toggles.FPSBoost_AF then return end
...
local function HybridMove(targetCF)
    ...
    local tweenSpeed = Options.TweenSpeed or 180

    if distance > tonumber(Options.TargetDistTP) then
...
local function SafeTeleportToNPC(targetName, customMap)
    ...
    if target then
        ...
        Library.CreateNoti({
    Title = "Silver Hub",       
    Desc = "NPC not found: " .. tostring(actualName), 
    ShowTime = 5        
})
    end
end
...
local function EvaluateArtifact2(uuid, data)
    ...
    if Toggles.ArtifactUpgrade and data.Level < Options.UpgradeLimit then
    ...
    local lockMinSS = Options.Lock_MinSS
    if Toggles.ArtifactLock and not data.Locked and data.Level >= (lockMinSS * 3) then
        if IsWhitelisted(Options.Lock_MS, data.MainStat.Stat) and
           IsWhitelisted(Options.Lock_Type, data.Category) and
           IsWhitelisted(Options.Lock_Set, data.Set) then
            if GetMatches(data, Options.Lock_SS) >= lockMinSS then
                actions.lock = true
            end
        end
    end
    ...
    if not data.Locked and not actions.lock then
        if Toggles.DeleteUnlock then
            actions.delete = true
        elseif Toggles.ArtifactDelete then
            
            local typeMatch = GetFilterStatus(Options.Del_Type, data.Category)
            local setMatch = GetFilterStatus(Options.Del_Set, data.Set)

            -- Check type-specific Main Stat
            local msDropdownName = "Del_MS_" .. data.Category
                local specificMSFilter = Options[msDropdownName] or {}
            ...
            if isTarget then
                local trashCount = GetMatches(data, Options.Del_SS)
                local minTrash = Options.Del_MinSS
                local isMaxLevel = data.Level >= Options.UpgradeLimit
...
local function AutoEquipArtifacts()
    ...
    local targetTypes = Options.Eq_Type or {}
    local targetMS = Options.Eq_MS or {}
    local targetSS = Options.Eq_SS or {}
...
local function AutoUpgradeLoop(mode)
    ...
    ShowTime = 5        
            ...
            Toggles["Auto"..mode] = false
            Toggles["Auto"..mode.."All"] = false
            toggle = false
            allToggle = false
            break
        end
...
local function HandleSummons()
    ...
    if Toggles.PityBossFarm then
        ...
        local buildOptions = Options.SelectedBuildPity or {} 
    ...
    if Toggles.AutoSummon then
        ...
        if selected then
            ...
            if not found then
                FireBossRemote(selected, Options.SelectedSummonDiff or "Normal")
                task.wait(0.5)
            end
        end
    end
...
local function UpdateSwitchState(target, farmType)
    ...
    for _, switch in ipairs(types) do
        ...
        if not toggleObj then continue end
        ...
        local toEquip = ""
        local threshold = Options[switch.id.."_BossHPAmt"]
        local isLow = false
...
local function ProcessNotification(frame)
    task.delay(0.01, function()
        if not Toggles.AutoDeleteNotif then return end
...
local function AutoQuestlineLoop()
    while Toggles.AutoQuestline do
        ...
        local selectedId = Options.SelectedQuestline
        ...
        if currentStage then
            ...
            if taskType == "CombatNPCKills" or taskType == "CombatPunches" or taskType == "GroundSmashUses" then
                ...
                Options.SelectedMob = {["Thief"] = true}
                Toggles.MobFarm = true
            ...
            elseif taskType:find("Kills") and taskType ~= "PlayerKills" and not taskType:find("Boss") then
                ...
                if mobName == "AnyNPC" then
                    Toggles.LevelFarm = true
                elseif mobName == "HakiNPC" then
                    Toggles.ArmHaki = true 
                    Toggles.LevelFarm = true
                else
            ...
            elseif taskType == "DamageTaken" then
                local targetName = Options.SelectedQuestline_DMGTaken
                if targetName then
            ...
            elseif taskType == "PlayerKills" then
                local targetName = Options.SelectedQuestline_Player
            ...
            elseif taskType:find("BossKills") or taskType == "AnyBossKills" then
                if taskType == "AnyBossKills" then
                    ...
                    if table.find(Tables.MiniBossList, bossName) then
                        Options.SelectedMob = {[bossName] = true}
                        Toggles.MobFarm = true
                    else
                        ...
                        if isRegular then
                            Options.SelectedSummon = bossName
                            Options.SelectedSummonDiff = diff
                            Toggles.AutoSummon = true
                            Toggles.SummonBossFarm = true
                        elseif isOther then
                            Options.SelectedOtherSummon = bossName
                            Options.SelectedOtherSummonDiff = diff
                            Toggles.AutoOtherSummon = true
                            Toggles.OtherSummonFarm = true
                        else
                            Options.SelectedBosses = {[bossName] = true}
                            Toggles.BossesFarm = true
                        end
                    end
                end
...
local function GetMobTarget()
    ...
    local selectedDict = Options.SelectedMob or {}
    local enabledMobs = {}
...
local function GetAltHelpTarget()
    ...
    if not targetNPC then
        FireBossRemote(targetBossName, Options.SelectedAltDiff or "Normal")
        task.wait(0.5)
        return nil
    end
...
local function CheckTask(taskType)
    if taskType == "Merchant" then
        if Toggles.AutoMerchant and Shared.MerchantBusy then
...
local function GetNearestAuraTarget()
    local nearest = nil
    local maxRange = tonumber(Options.KillAuraRange) or 200
...
local function ExecuteFarmLogic(target, island, farmType)
    ...
    if Toggles.IslandTP then
        if island and island ~= "" and island ~= "Unknown" and island ~= Shared.Island then
            ...
            task.wait(tonumber(Options.IslandTPCD) or 0.8)
    ...
    local distVal = tonumber(Options.Distance) or 10
    local posType = Options.SelectedFarmType
    
    local finalPos
    local ikTag = target:FindFirstChild("IK_Active")

    if ikTag and Options.InstaKillType == "V2" and Toggles.InstaKill then
    ...
    if (root.Position - finalPos).Magnitude > 0.1 then
        if Options.SelectedMovementType == "Teleport" then
            root.CFrame = finalDestination
        else
            local distance = (root.Position - finalPos).Magnitude
            local speed = tonumber(Options.TweenSpeed) or 180
...
local function Func_WebhookLoop()
    while Toggles.SendWebhook do
        PostToWebhook()
        local delay = math.max(Options.WebhookDelay, 0.5) * 60
        task.wait(delay)
    end
end
...
local function Func_AutoHaki()
    while task.wait(0.5) do
        if Toggles.ObserHaki and not CheckObsHaki() then
            Remotes.ObserHaki:FireServer("Toggle")
        end

        if Toggles.ArmHaki and not CheckArmHaki() then
            Remotes.ArmHaki:FireServer("Toggle")
            task.wait(0.5) 
        end

        if Toggles.ConquerorHaki then
...
local function Func_AutoM1()
    while task.wait(Options.M1Speed) do
...
local function Func_AutoSkill()
    ...
    while task.wait() do
        ...
        if Toggles.OnlyTarget and (not Shared.Farm or not target or not target.Parent) then
            continue
        end

        local canExecute = true
        if Toggles.AutoSkill_BossOnly then
            if not target or not target.Parent then
                ...
                local threshold = tonumber(Options.AutoSkill_BossHP) or 100
        ...
        if canExecute and target and target.Parent then
            if target:FindFirstChild("IK_Active") and Options.InstaKillType == "V1" then
                canExecute = false
            end
        end
        ...
        local useMode = Options.AutoSkillType
...
local function Func_AutoCombo()
    ...
    while Toggles.AutoCombo do
        ...
        local rawPattern = Options.ComboPattern
        ...
        if waitTime then
            if Options.ComboMode == "Normal" then task.wait(waitTime) end
        ...
        if IsSkillReady(currentAction) then
            ...
            if isF then
                ...
                until uiConfirmed or not Toggles.AutoCombo
                ...
                until stepDone or not Toggles.AutoCombo
    ...
    while task.wait(1) do
        if Toggles.AutoStats then
            ...
            if availablePoints > 0 then
                local selectedStats = Options.SelectedStats
                local activeStats = {}
...
local function AutoRollStatsLoop()
    local selectedStats = Options.SelectedGemStats or {}
    local selectedRanks = Options.SelectedRank or {}
            ...
            if selectedStats[statName] then
                ...
                if currentData then
                    ...
                    if not selectedRanks[currentRank] then
                        ...
                        task.wait(tonumber(Options.StatsRollCD) or 0.1)
...
local function EnsureRollManager()
    Thread("UnifiedRollManager", Func_UnifiedRollManager, 
        Toggles.AutoTrait or Toggles.AutoRace or Toggles.AutoClan
    )
end
            ...
            if isCorrectName then
                if type(currentBuffs) == "table" then
                    for statKey, rolledValue in pairs(currentBuffs) do
                        ...
                        local minRequired = Options[sliderId] and Options[sliderId] or 0
...
local function Func_AutoDungeon()
    while Toggles.AutoDungeon do
        ...
        local selected = Options.SelectedDungeon
...
local function Func_AutoMerchant()
    ...
    local function StartPurchaseSequence()
        ...
        if #itemsWithStock > 0 then
            local selectedItems = Options.SelectedMerchantItems
...
local function Func_AutoTrade()
    while task.wait(0.5) do
        ...
        if Toggles.ReqTradeAccept and requestUI then
            Remotes.TradeRespond:FireServer(true)
            task.wait(1)
        end

        if Toggles.ReqTrade and not inTradeUI and not requestUI then
            local targetPlr = Options.SelectedTradePlr
        ...
        if inTradeUI and Toggles.AutoAccept then
            local selectedItems = Options.SelectedTradeItems or {}
            local itemsToAdd = {}
...
local function Func_AutoChest()
    while task.wait(2) do
        if not Toggles.AutoChest then break end

        local selected = Options.SelectedChests
...
local function Func_AutoCraft()
    while task.wait(1) do
        if Toggles.AutoCraftItem then
            local selected = Options.SelectedCraftItems
...
local function Func_ArtifactAutomation()
    while task.wait(5) do
        ...
        for uuid, data in pairs(Shared.ArtifactSession.Inventory) do
            ...
            if res.upgrade then
                local targetLvl = Options.UpgradeLimit
                if Toggles.UpgradeStage then
                    targetLvl = math.min(math.floor(data.Level / 3) * 3 + 3, Options.UpgradeLimit)
                end
...
local function CreateNoti(title, desc, time)
    Library.CreateNoti({
        Title = title,
        Desc = desc,
        ShowTime = time or 3
...
-- ============ AUTOFARM LOGIC ============
local MobList = {
    "Bandit", "Brute", "Guardian", "Mage", "Archer",
    "Warrior", "Assassin", "Berserker", "Paladin", "Rogue"
}

local BossList = {
    "Goblin King", "Dragon Lord", "Shadow Beast", "Ice Giant", "Fire Demon"
}
...
local function AutoM1Loop()
    while Toggles.AutoM1 and getgenv().taolao_Running do
        ...
        if target then
            ...
            if root and mobRoot then
                -- Move to target
                if (root.Position - mobRoot.Position).Magnitude > 10 then
                    root.CFrame = CFrame.new(mobRoot.Position + Vector3.new(0, 3, 0))
                end
                
                -- Attack
                if tick() - Shared.LastM1 >= Options.M1Speed then
                    game:GetService("ReplicatedStorage").Remotes.M1:FireServer()
                    Shared.LastM1 = tick()
                end
            end
        end
...
local function KillAuraLoop()
    while Toggles.KillAura and getgenv().taolao_Running do
        local target = FindNearestMob()
        if target then
            local mobRoot = target:FindFirstChild("HumanoidRootPart")
            local root = GetRootPart()
            if mobRoot and root then
                ...
                if dist <= Options.KillAuraRange then
                    game:GetService("ReplicatedStorage").Remotes.M1:FireServer()
                end
            end
        end
...
local function AutoSkillLoop()
    while Toggles.AutoSkill and getgenv().taolao_Running do
        local target = FindNearestMob()
        if target then
            for _, skill in pairs(Options.SelectedSkills) do
                game:GetService("ReplicatedStorage").Remotes.Skill:FireServer(skill)
                task.wait(0.15)
            end
        end
        task.wait(0.5)
...
local function AutoHakiLoop()
    while (Toggles.ObserHaki or Toggles.ArmHaki or Toggles.ConquerorHaki) and getgenv().taolao_Running do
        if Toggles.ObserHaki then
            game:GetService("ReplicatedStorage").Remotes.Haki:FireServer("Observation")
        end
        if Toggles.ArmHaki then
            game:GetService("ReplicatedStorage").Remotes.Haki:FireServer("Armament")
        end
        if Toggles.ConquerorHaki then
            game:GetService("ReplicatedStorage").Remotes.Haki:FireServer("Conqueror")
        end
        task.wait(0.5)
    end
end
...
local function AutoStatsLoop()
    while Toggles.AutoStats and getgenv().taolao_Running do
        for _, stat in pairs(Options.SelectedStats) do
            game:GetService("ReplicatedStorage").Remotes.UpStat:FireServer(stat)
            task.wait(0.2)
        end
        task.wait(1)
    end
...
local function AutoAscendLoop()
    while Toggles.AutoAscend and getgenv().taolao_Running do
        game:GetService("ReplicatedStorage").Remotes.ReqAscend:InvokeServer()
        task.wait(5)
    end
end
...
local function AutoDungeonLoop()
    while Toggles.AutoDungeon and getgenv().taolao_Running do
        game:GetService("ReplicatedStorage").Remotes.JoinDungeon:FireServer(Options.SelectedDungeon)
        task.wait(10)
    end
end
...
local function AntiAFKLoop()
    while Toggles.AntiAFK and getgenv().taolao_Running do
        local vuser = game:GetService("VirtualUser")
        vuser:CaptureController()
        vuser:ClickButton2(Vector2.new(0, 0))
        task.wait(60)
    end
end
...
local function NoclipLoop()
    while Toggles.Noclip and getgenv().taolao_Running do
        local char = GetCharacter()
        if char then
            for _, part in pairs(char:GetDescendants()) do
...
local function FullbrightLoop()
    while Toggles.Fullbright and getgenv().taolao_Running do
        local lighting = game:GetService("Lighting")
...
local function AutoChestLoop()
    while Toggles.AutoChest and getgenv().taolao_Running do
        for _, chest in pairs(workspace:GetChildren()) do
            if chest.Name:find("Chest") and chest:FindFirstChild("ProximityPrompt") then
                local root = GetRootPart()
                if root and (root.Position - chest.Position).Magnitude < 15 then
...
local function AutoCraftLoop()
    while Toggles.AutoCraftItem and getgenv().taolao_Running do
        for _, item in pairs(Options.CraftItems) do
            game:GetService("ReplicatedStorage").Remotes.CraftItem:FireServer(item)
            task.wait(0.5)
        end
        task.wait(2)
    end
...
local function AutoMerchantLoop()
    while Toggles.AutoMerchant and getgenv().taolao_Running do
        for _, item in pairs(Options.MerchantItems) do
            game:GetService("ReplicatedStorage").Remotes.BuyItem:FireServer(item)
            task.wait(0.3)
        end
        task.wait(5)
    end
...
local function SendWebhook(content)
    if Options.WebhookURL == "" then return end
    
    local data = {
        content = content,
        username = "Silver Hub Webhook"
    }
    
    if Options.WebhookUserID ~= "" then
        data.content = "<@" .. Options.WebhookUserID .. "> " .. data.content
    end
...
local function WebhookLoop()
    while Toggles.SendWebhook and getgenv().taolao_Running do
        local player = game.Players.LocalPlayer
        local dataToSend = {}
        
        if table.find(Options.WebhookData, "Name") then
            table.insert(dataToSend, "Name: " .. player.Name)
        end
        if table.find(Options.WebhookData, "Stats") then
            table.insert(dataToSend, "Level: " .. (player.Data.Level.Value or 0))
        end
        
        if #dataToSend > 0 then
            SendWebhook(table.concat(dataToSend, "\n"))
        end
...
-- ============ AUTO ROLL LOGIC ============
local function AutoRollLoop(type, selectedList)
    while (type == "trait" and Toggles.AutoTrait) or
          (type == "race" and Toggles.AutoRace) or
          (type == "clan" and Toggles.AutoClan) do
        game:GetService("ReplicatedStorage").Remotes.Roll:FireServer(type)
        task.wait(0.5)
    end
end
...
local IslandLocations = {
    ...
    ["Snow Island"] = Vector3.new(-1000, 10, 0),
    ["Sky Island"] = Vector3.new(0, 500, 0)
}

local NPCLocations = {
    ["Quest NPC"] = Vector3.new(50, 10, 50),
    ["Shop NPC"] = Vector3.new(-50, 10, -50),
    ["Upgrade NPC"] = Vector3.new(100, 10, 0),
    ["Bank NPC"] = Vector3.new(0, 10, 100)
}

local function TeleportToIsland(island)
    if IslandLocations[island] then
        TeleportToPosition(IslandLocations[island])
        CreateNoti("Teleported", "To " .. island, 2)
    end
end

local function TeleportToNPC(npc)
    if NPCLocations[npc] then
        TeleportToPosition(NPCLocations[npc])
        CreateNoti("Teleported", "To " .. npc, 2)
    end
end
...
local function PanicMode()
    ...
    TeleportToPosition(Vector3.new(0, 500, 0))
    
    CreateNoti("PANIC MODE", "All features disabled!", 5)
end

-- ============ UPDATE LOOPS ============
local function UpdateToggles()
    -- Autofarm toggles
    if Toggles.MobFarm or Toggles.AllMobFarm or Toggles.BossesFarm then
        Shared.Farm = true
    else
        Shared.Farm = false
        Shared.Target = nil
...
-- Information Page
local InfoUser = Information.CreateSection("User")
local InfoGame = Information.CreateSection("Game")
local InfoOthers = Information.CreateSection("Others")

InfoUser.CreateLabel({ Title = "<b>Type:</b> Premium User" })
InfoUser.CreateLabel({ Title = "<b>Time Left:</b> Lifetime / Permanent" })
InfoUser.CreateLabel({ Title = "<b>Executor:</b> " .. (identifyexecutor and identifyexecutor() or "Unknown") })

InfoGame.CreateButton({ Title = "Redeem All Codes" }, function()
    CreateNoti("Codes", "Attempting to redeem all codes...", 3)
    ...
    for codeName, data in pairs(allCodes) do
        ...
        if playerLevel >= levelReq then
            ...
            CreateNoti("Attempt to redeem code: " .. codeName .. rewardText, 8)
            
            Remotes.UseCode:InvokeServer(codeName)
            
            task.wait(2)
        else
            CreateNoti(string.format("Not enough requirement for: %s (Req. Lvl %d)", codeName, levelReq), 4)
        end
    end
    -- Code redeem logic
end)

InfoOthers.CreateLabel({ Title = "⚠️ If some features are disabled, it is because your executor lacks the required functions." })
InfoOthers.CreateButton({ Title = "Copy Discord Invite" }, function()
    if setclipboard then
        setclipboard("https://discord.gg/wEZmsW7BXp")
        CreateNoti("Copied!", "Discord invite copied", 2)
    end
end)

-- Priority Page
local PrioritySection = Priority.CreateSection("Priority Config")
for i = 1, 5 do
    PrioritySection.CreateDropdown({
        Title = "Priority " .. i,
        Options = {"Mob", "Boss", "Quest", "Merchant", "Chest"},
        Default = "Mob"
    }, function(Value)
        Options["Priority_" .. i] = Value
    end)
end

-- Autofarm Page
local AutoMain = Autofarm.CreateSection("Main Settings")
local AutoConfig = Autofarm.CreateSection("Config Settings")

AutoMain.CreateDropdown({
    Title = "Select Mob (s)",
    Options = MobList,
    Multi = true
}, function(Value)
    Options.SelectedMob = Value
end)

AutoMain.CreateButton({ Title = "Refresh Mobs" }, function()
    CreateNoti("Refreshed", "Mob list updated", 1)
end)

AutoMain.CreateToggle({ Title = "Autofarm Selected Mob", Default = false }, function(Value)
    Toggles.MobFarm = Value
    UpdateToggles()
    if Value then
        CreateNoti("Autofarm", "Started farming selected mobs", 2)
        task.spawn(AutoM1Loop)
    end
end)

AutoMain.CreateToggle({ Title = "Autofarm Level", Default = false }, function(Value)
    Toggles.LevelFarm = Value
end)

AutoConfig.CreateSlider({ Title = "M1 Attack Cooldown", Min = 0, Max = 1, Default = 0.2, Precise = true }, function(Value)
    Options.M1Speed = Value
end)

AutoConfig.CreateSlider({ Title = "Tween Speed", Min = 0, Max = 500, Default = 160 }, function(Value)
    Options.TweenSpeed = Value
end)

AutoConfig.CreateDropdown({ Title = "Movement Type", Options = {"Teleport", "Tween"}, Default = "Tween" }, function(Value)
    Options.SelectedMovementType = Value
end)

AutoConfig.CreateToggle({ Title = "Instant Kill", Default = false }, function(Value)
    Toggles.InstaKill = Value
    if Value then
        CreateNoti("Warning", "Instant Kill enabled!", 2)
    end
end)

-- Boss Farm Page
local BossMain = BossFarm.CreateSection("Boss Settings")

BossMain.CreateDropdown({
    Title = "Select Bosses",
    Options = BossList,
    Multi = true
}, function(Value)
    Options.SelectedBosses = Value
end)

BossMain.CreateToggle({ Title = "Autofarm Selected Boss", Default = false }, function(Value)
    Toggles.BossesFarm = Value
    UpdateToggles()
end)

BossMain.CreateToggle({ Title = "Autofarm All Bosses", Default = false }, function(Value)
    Toggles.AllBossesFarm = Value
end)

local PityLabel = BossMain.CreateLabel({ Title = "<b>Pity:</b> 0/25" })

-- Automation Page
local AutoHaki = Automation.CreateSection("Haki")
local AutoSkillSec = Automation.CreateSection("Skills")
local AutoStatsSec = Automation.CreateSection("Stats")
local AutoRollSec = Automation.CreateSection("Rolls")
local AutoAscendSec = Automation.CreateSection("Ascend")

AutoHaki.CreateToggle({ Title = "Auto Observation Haki", Default = false }, function(Value)
    Toggles.ObserHaki = Value
    if Value then task.spawn(AutoHakiLoop) end
end)

AutoHaki.CreateToggle({ Title = "Auto Armament Haki", Default = false }, function(Value)
    Toggles.ArmHaki = Value
    if Value then task.spawn(AutoHakiLoop) end
end)

AutoHaki.CreateToggle({ Title = "Auto Conqueror Haki", Default = false }, function(Value)
    Toggles.ConquerorHaki = Value
    if Value then task.spawn(AutoHakiLoop) end
end)

AutoSkillSec.CreateToggle({ Title = "Auto Attack (M1)", Default = false }, function(Value)
    Toggles.AutoM1 = Value
    if Value then task.spawn(AutoM1Loop) end
end)

AutoSkillSec.CreateToggle({ Title = "Kill Aura", Default = false }, function(Value)
    Toggles.KillAura = Value
    if Value then task.spawn(KillAuraLoop) end
end)

AutoSkillSec.CreateToggle({ Title = "Auto Use Skills", Default = false }, function(Value)
    Toggles.AutoSkill = Value
    if Value then task.spawn(AutoSkillLoop) end
end)

AutoSkillSec.CreateDropdown({
    Title = "Select Skills",
    Options = {"Z", "X", "C", "V", "F"},
    Multi = true,
    Default = {"Z", "X", "C"}
...
AutoSkillSec.CreateSlider({ Title = "Kill Aura Range", Min = 0, Max = 200, Default = 200 }, function(Value)
    Options.KillAuraRange = Value
end)

AutoStatsSec.CreateToggle({ Title = "Auto UP Stats", Default = false }, function(Value)
    Toggles.AutoStats = Value
    if Value then task.spawn(AutoStatsLoop) end
end)

AutoStatsSec.CreateToggle({ Title = "Auto Roll Stats", Default = false }, function(Value)
    Toggles.AutoRollStats = Value
end)

AutoStatsSec.CreateToggle({ Title = "Auto Skill Tree", Default = false }, function(Value)
    Toggles.AutoSkillTree = Value
end)

AutoStatsSec.CreateDropdown({
    Title = "Select Stat (s)",
    Options = {"Melee", "Defense", "Sword", "Power"},
    Multi = true
}, function(Value)
    Options.SelectedStats = Value
end)

AutoRollSec.CreateToggle({ Title = "Auto Roll Trait", Default = false }, function(Value)
    Toggles.AutoTrait = Value
    if Value then task.spawn(function() AutoRollLoop("trait", Options.SelectedTrait) end) end
end)

AutoRollSec.CreateToggle({ Title = "Auto Roll Race", Default = false }, function(Value)
    Toggles.AutoRace = Value
    if Value then task.spawn(function() AutoRollLoop("race", Options.SelectedRace) end) end
end)

AutoRollSec.CreateToggle({ Title = "Auto Roll Clan", Default = false }, function(Value)
    Toggles.AutoClan = Value
    if Value then task.spawn(function() AutoRollLoop("clan", Options.SelectedClan) end) end
end)

AutoRollSec.CreateDropdown({
    Title = "Select Trait (s)",
    Options = {"Godly", "Legendary", "Epic", "Rare"},
    Multi = true
}, function(Value)
    Options.SelectedTrait = Value
end)

AutoAscendSec.CreateToggle({ Title = "Auto Ascend", Default = false }, function(Value)
    Toggles.AutoAscend = Value
    if Value then
        task.spawn(AutoAscendLoop)
        CreateNoti("Ascend", "Auto ascend started", 2)
    end
end)

-- Artifact Page
local ArtiStatus = Artifact.CreateSection("Status")
local ArtiEquip = Artifact.CreateSection("Auto-Equip")
local ArtiUpgrade = Artifact.CreateSection("Upgrade")
local ArtiLock = Artifact.CreateSection("Lock")
local ArtiDelete = Artifact.CreateSection("Delete")

ArtiStatus.CreateLabel({ Title = "Status: N/A" })
ArtiStatus.CreateLabel({ Title = "Dust: N/A" })
ArtiStatus.CreateLabel({ Title = "Helmet: 0/500" })
ArtiStatus.CreateLabel({ Title = "Gloves: 0/500" })
ArtiStatus.CreateLabel({ Title = "Body: 0/500" })
ArtiStatus.CreateLabel({ Title = "Boots: 0/500" })

ArtiEquip.CreateToggle({ Title = "Auto Equip Best Artifacts", Default = false }, function(Value)
    Toggles.ArtifactEquip = Value
end)

ArtiEquip.CreateDropdown({
    Title = "Artifact Type",
    Options = {"Helmet", "Gloves", "Body", "Boots"},
    Multi = true
}, function(Value)
    Options.Eq_Type = Value
end)

ArtiUpgrade.CreateSlider({ Title = "Upgrade Limit", Min = 0, Max = 15, Default = 0 }, function(Value)
    Options.UpgradeLimit = Value
end)

ArtiUpgrade.CreateToggle({ Title = "Auto Upgrade", Default = false }, function(Value)
    Toggles.ArtifactUpgrade = Value
end)

ArtiLock.CreateToggle({ Title = "Auto Lock Good Artifacts", Default = false }, function(Value)
    Toggles.ArtifactLock = Value
end)

ArtiLock.CreateDropdown({ Title = "Main Stat Filter", Options = {"ATK%", "HP%", "DEF%", "Crit Rate"}, Multi = true }, function(Value)
    Options.Lock_MS = Value
end)

ArtiDelete.CreateToggle({ Title = "Auto Delete Bad Artifacts", Default = false }, function(Value)
    Toggles.ArtifactDelete = Value
end)

Artifact.CreateLabel({ Title = "⚠️ WARNING: Artifact features are in beta. Use at your own risk!" })

-- Dungeon Page
local DungeonMain = Dungeon.CreateSection("Dungeon Settings")

DungeonMain.CreateDropdown({
    Title = "Select Dungeon",
    Options = {"BossRush", "Trial Tower", "Raid"},
    Default = "BossRush"
}, function(Value)
    Options.SelectedDungeon = Value
end)

DungeonMain.CreateToggle({ Title = "Auto Join Dungeon", Default = false }, function(Value)
    Toggles.AutoDungeon = Value
    if Value then task.spawn(AutoDungeonLoop) end
end)

DungeonMain.CreateToggle({ Title = "Auto Replay", Default = false }, function(Value)
    Toggles.AutoReplay = Value
end)

DungeonMain.CreateToggle({ Title = "Autofarm Dungeon", Default = false }, function(Value)
    Toggles.DungeonAutofarm = Value
end)

local DungeonCount = DungeonMain.CreateLabel({ Title = "Dungeon Completed: 0" })

-- Player Page
local PlayerGeneral = Player.CreateSection("General")
local PlayerServer = Player.CreateSection("Server")
local PlayerGame = Player.CreateSection("Game")
local PlayerSafety = Player.CreateSection("Safety")

PlayerGeneral.CreateSlider({ Title = "WalkSpeed", Min = 16, Max = 250, Default = 16 }, function(Value)
    Options.WSValue = Value
    if Toggles.WS then
...
PlayerGeneral.CreateToggle({ Title = "Enable WalkSpeed", Default = false }, function(Value)
    Toggles.WS = Value
    local hum = GetHumanoid()
    if hum then
        hum.WalkSpeed = Value and Options.WSValue or 16
    end
end)

PlayerGeneral.CreateSlider({ Title = "JumpPower", Min = 0, Max = 500, Default = 50 }, function(Value)
    Options.JPValue = Value
    if Toggles.JP then
        local hum = GetHumanoid()
...
PlayerGeneral.CreateToggle({ Title = "Enable JumpPower", Default = false }, function(Value)
    Toggles.JP = Value
    local hum = GetHumanoid()
    if hum then
        hum.JumpPower = Value and Options.JPValue or 50
    end
end)

PlayerGeneral.CreateToggle({ Title = "Noclip", Default = false }, function(Value)
    Toggles.Noclip = Value
    if Value then task.spawn(NoclipLoop) end
end)

PlayerGeneral.CreateToggle({ Title = "Anti Knockback", Default = false }, function(Value)
    Toggles.AntiKnockback = Value
end)

PlayerGeneral.CreateToggle({ Title = "FPS Boost", Default = false }, function(Value)
...
PlayerServer.CreateToggle({ Title = "Anti AFK", Default = true }, function(Value)
    Toggles.AntiAFK = Value
    if Value then task.spawn(AntiAFKLoop) end
end)

PlayerServer.CreateToggle({ Title = "Auto Reconnect", Default = false }, function(Value)
    Toggles.AutoReconnect = Value
end)

PlayerServer.CreateButton({ Title = "Server Hop" }, function()
    CreateNoti("Server Hop", "Looking for new server...", 2)
...
PlayerServer.CreateButton({ Title = "Rejoin" }, function()
    game:GetService("TeleportService"):Teleport(game.PlaceId, game.Players.LocalPlayer)
end)

PlayerGame.CreateToggle({ Title = "Instant Prompt", Default = false }, function(Value)
    Toggles.InstantPP = Value
    if Value then
...
PlayerGame.CreateToggle({ Title = "Fullbright", Default = false }, function(Value)
    Toggles.Fullbright = Value
    if Value then task.spawn(FullbrightLoop) end
end)

PlayerGame.CreateToggle({ Title = "No Fog", Default = false }, function(Value)
    Toggles.NoFog = Value
    if Value then
...
PlayerSafety.CreateLabel({ Title = "Panic Button - Disables all features" })
PlayerSafety.CreateButton({ Title = "PANIC MODE" }, function()
    PanicMode()
end)

PlayerSafety.CreateToggle({ Title = "Auto Kick on Mod Join", Default = true }, function(Value)
    Toggles.AutoKick = Value
end)

-- Teleport Page
local TeleIsland = Teleport.CreateSection("Island Teleport")
local TeleNPC = Teleport.CreateSection("NPC Teleport")

TeleIsland.CreateDropdown({
    Title = "Select Island",
    Options = {"Starter Island", "Desert Island", "Snow Island", "Sky Island"}
}, function(Value)
    if Value then TeleportToIsland(Value) end
end)

TeleNPC.CreateDropdown({
    Title = "Select NPC",
    Options = {"Quest NPC", "Shop NPC", "Upgrade NPC", "Bank NPC"}
}, function(Value)
    if Value then TeleportToNPC(Value) end
end)

-- Webhook Page
local WebhookSection = Webhook.CreateSection("Webhook Config")

WebhookSection.CreateBox({
    Title = "Webhook URL",
...
WebhookSection.CreateBox({
    ...
    Options.WebhookUserID = Value
end)

WebhookSection.CreateDropdown({
    Title = "Data to Send",
    Options = {"Name", "Stats", "New Items", "All Items"},
    Multi = true
}, function(Value)
    Options.WebhookData = Value
end)

WebhookSection.CreateToggle({ Title = "Send Webhook", Default = false }, function(Value)
    Toggles.SendWebhook = Value
    if Value then task.spawn(WebhookLoop) end
end)

WebhookSection.CreateButton({ Title = "Test Webhook" }, function()
    SendWebhook("Test message from Silver Hub!")
    CreateNoti("Test", "Webhook sent!", 2)
end)

-- Misc Page
local MiscChest = Misc.CreateSection("Chests")
local MiscCraft = Misc.CreateSection("Crafting")
local MiscMerchant = Misc.CreateSection("Merchant")
local MiscQuest = Misc.CreateSection("Quests")

MiscChest.CreateToggle({ Title = "Auto Open Chests", Default = false }, function(Value)
    Toggles.AutoChest = Value
    if Value then task.spawn(AutoChestLoop) end
end)

MiscChest.CreateDropdown({
    Title = "Chest Rarity",
    Options = {"Common", "Rare", "Epic", "Legendary"},
    Multi = true
}, function(Value)
    Options.SelectedChests = Value
end)

MiscCraft.CreateToggle({ Title = "Auto Craft Items", Default = false }, function(Value)
    Toggles.AutoCraftItem = Value
    if Value then task.spawn(AutoCraftLoop) end
end)

MiscCraft.CreateDropdown({
    Title = "Items to Craft",
    Options = {"Sword", "Armor", "Potion", "Scroll"},
    Multi = true
}, function(Value)
    Options.CraftItems = Value
end)

MiscMerchant.CreateToggle({ Title = "Auto Buy Items", Default = false }, function(Value)
    Toggles.AutoMerchant = Value
    if Value then task.spawn(AutoMerchantLoop) end
end)

MiscMerchant.CreateDropdown({
    Title = "Items to Buy",
    Options = {"Health Potion", "Mana Potion", "Upgrade Stone"},
    Multi = true
}, function(Value)
    Options.MerchantItems = Value
end)

MiscQuest.CreateToggle({ Title = "Auto Questline", Default = false }, function(Value)
    Toggles.AutoQuestline = Value
end)

MiscQuest.CreateButton({ Title = "Complete Dungeon Puzzle" }, function()
    CreateNoti("Puzzle", "Solving dungeon puzzle...", 2)
end)

-- Config Page
local ConfigUI = Config.CreateSection("UI Settings")
local ConfigScript = Config.CreateSection("Script Settings")

ConfigUI.CreateToggle({ Title = "Auto Show UI on Load", Default = true }, function(Value)
    Options.AutoShowUI = Value
end)

ConfigUI.CreateDropdown({
    Title = "Notification Position",
    Options = {"Left", "Right"},
    Default = "Right"
}, function(Value)
    Options.NotificationSide = Value
...
ConfigUI.CreateDropdown({
    ...
    Options.DPIScale = dpi
    Library:SetDPIScale(dpi)
end)

ConfigScript.CreateButton({ Title = "Save Config" }, function()
    CreateNoti("Saved", "Configuration saved!", 2)
    -- Save config logic here
end)

ConfigScript.CreateButton({ Title = "Load Config" }, function()
    CreateNoti("Loaded", "Configuration loaded!", 2)
    -- Load config logic here
end)

ConfigScript.CreateButton({ Title = "Unload Script" }, function()
    CreateNoti("Unloading", "Script will be unloaded...", 2)
    getgenv().taolao_Running = false
    task.wait(1)
    if game.CoreGui:FindFirstChild("Silver Hub GUI") then
        game.CoreGui:FindFirstChild("Silver Hub GUI"):Destroy()
    end
    if game.CoreGui:FindFirstChild("Silver Hub Notification") then
        game.CoreGui:FindFirstChild("Silver Hub Notification"):Destroy()
    end
end)

-- ============ FINAL NOTIFICATION ============
CreateNoti("Silver Hub", "UI Loaded Successfully!", 3)
